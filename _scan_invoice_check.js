...
function openCreateProductModal(tr) {
    activeCreateRow = tr;
    const nombre = tr.querySelector('[data-field="nombre"]').value.trim();
    lastEditedPriceField = 'margin'; // Reset direcciÃ³n al abrir el modal
...
async function createProductFromScan() {
    ...
    const sale_price = parseFloat(document.getElementById('cpSalePrice').value) || 0;
    ...
    if (sale_price <= 0) {
        errorDiv.textContent = 'El precio de venta debe ser mayor a 0.';
        errorDiv.classList.remove('d-none');
        return;
    }

    if (!activeCreateRow) {
        errorDiv.textContent = 'No se pudo identificar la fila del producto a vincular. CerrÃ¡ y abrÃ­ el modal nuevamente.';
        errorDiv.classList.remove('d-none');
        return;
    }
